...
Bun includes a fast, built-in Markdown parser written in Zig. It supports GitHub Flavored Markdown (GFM) extensions and provides four APIs:

* `Bun.markdown.html()` — render Markdown to an HTML string
* `Bun.markdown.ansi()` — render Markdown to ANSI-formatted terminal output
...
| `autolinks`            | `false` | Enable autolinks — see Autolinks section                    |
| `headings`             | `false` | Heading IDs and autolinks — see Heading IDs section         |
...
Render Markdown to ANSI-formatted text suitable for terminal output. Added in Bun 1.3.14.

```ts
const output = Bun.markdown.ansi("# Hello **world**");
// Prints styled text to the terminal with ANSI escape codes
console.log(output);
```

### AnsiTheme

Pass an `AnsiTheme` object to customize the terminal rendering:

```ts
const output = Bun.markdown.ansi("# Hello **world**", {
  colors: true,        // Enable ANSI color/styling escape codes (default: true)
  columns: 80,         // Line width for word-wrapping (0 disables wrapping)
  hyperlinks: true,    // Enable OSC 8 clickable links in modern terminals
  kittyGraphics: false, // Enable inline images via Kitty Graphics Protocol
  light: false,        // Whether terminal background is light (auto-detected via COLORFGBG)
});
```

| Property | Type | Description |
|----------|------|-------------|
| `colors` | `boolean` | Controls ANSI color and styling escape sequences. When `false`, uses plain ASCII with no box drawing, no emoji, no escape codes |
| `columns` | `number` | Line width for word-wrapping paragraphs and headings and for horizontal rules. `0` disables wrapping |
| `hyperlinks` | `boolean` | Controls OSC 8 hyperlink output (clickable links). When `false`, links render as `text (url)` |
| `kittyGraphics` | `boolean` | Enables inline images via Kitty Graphics Protocol when `src` resolves to a local file. Supported in Kitty, WezTerm, and Ghostty |
| `light` | `boolean` | Whether terminal background is light. Affects color palette for inline code backgrounds. Defaults to detecting from `COLORFGBG` env var |
...
| `listItem`   | `{ index, depth, ordered, start?, checked? }` | List item metadata                                                                       |
| `hr`         | —                                             | Horizontal rule                                                                          |
| `table`      | —                                             | Table block                                                                              |
| `thead`      | —                                             | Table head                                                                               |
| `tbody`      | —                                             | Table body                                                                               |
