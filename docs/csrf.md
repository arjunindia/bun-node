...
Generates a CSRF token containing a cryptographic nonce, a timestamp, and an HMAC signature, encoded as a string.
...
- **`secret`** (`string`, optional) — The secret key used to sign the token. When omitted, Bun produces a random in-memory default secret unique per thread.
...
Verifies a CSRF token. Returns `true` if the token is valid and has not expired, `false` otherwise.
...
| Option | Type | Default | Description |
|---|---|---|---|
| `secret` | `string` | (auto) | The secret used to sign the token. If not provided, uses the same per-thread in-memory default as `generate()`. |
...
A typical pattern is to generate a token when rendering a form, embed it in a hidden field, and verify it upon form submission.
...
Omitting the `secret` parameter in both `generate()` and `verify()` causes Bun to use a random secret generated once per thread. This works for single-thread applications but is unsuitable across multiple servers, workers, or after restarts.
