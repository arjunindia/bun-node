...
Synchronous versions of all functions are also available:
...
Additional hashing algorithms are available as properties on `Bun.hash`:
...
Once initialized, data can be incrementally fed to the hasher using `.update()`. This method accepts `string`, `TypedArray`, and `ArrayBuffer`.
...
After the data has been fed into the hasher, a final hash can be computed using `.digest()`:
...
The `.digest()` method can optionally return the hash as a string:
...
Alternatively, the method can write the hash into a pre-existing `TypedArray` instance:
